...
var header = {
  scrolling: function scrolling(scroll) {
    if (scroll > 100) {
  ...
  closeEnother: function closeEnother(name) {
    if (name != "burger") {
  ...
  burgerOpen: function burgerOpen() {
    var _this = this;
...
var custom = {
  animateHover: {
    events: function events() {
      $(".hoverLineMoved,.plusBox_link,.btn-orange,.btn-gray,.news_item,.btn-orange-white,.btn-gray-white,.btn-orange-gray").hover(function () {
        $(this).addClass("ready");
      });
    },
    init: function init() {
  ...
  animateScroll: {
    checkAndDo: function checkAndDo() {
      var count = 0;

      var start_anim = function start_anim(item) {
        if (count > 5) {
      ...
      $(document).find(".is-hidden.anim-item").each(function () {
        if ($(this).offset().top < $(document).scrollTop() + $(window).height() && $(this).offset().top > $(document).scrollTop()) {
          start_anim($(this));
        }
      });
    },
  ...
  animateZoomOut: {
    zoomed: function zoomed() {
      $(document).find(".zoom-item").each(function () {
        var zoom;
        var paralaxY = 0;
        var k = 1;
        ...
        if ($(this).hasClass("in")) {
          ...
          var pos = $(document).scrollTop() - $(this).offset().top;
          /* - $(this).height() */
  ...
  animateFadeInLeftString: {
    create: function create() {
      $(document).find(".string-anim,.string-anim-content > p").each(function () {
        var item = this;
        ...
        stringAr.forEach(function (line) {
          item.innerHTML += '<span class="string-anim-paralax"><span class="string-anim-line is-hidden">' + line + "</span></span>";
        });
        temp.remove();
      });
    },
    checkAndDo: function checkAndDo() {
      ...
      $(document).find(".is-hidden.string-anim-line").each(function () {
        if ($(this).offset().top < $(document).scrollTop() + $(window).height() && $(this).offset().top > $(document).scrollTop()) {
          start_anim($(this));
        }
      });
  ...
  animateParalaxString: {
    paralax: function paralax() {
      $(document).find(".paralax-child").each(function () {
        var count = 0;
        $(this).find(".string-anim-paralax").each(function () {
          var pos = $(this).offset().top + $(this).height() / 2 - $(document).scrollTop() - $(window).height() / 2;
          var k = 70 + count * 10;
          var paralax = pos / $(window).height() * k;
          if (Math.abs(paralax) < 70) $(this).css("transform", "translateY(" + paralax + "px)");
          count++;
  ...
  animateScaleIn: {
    checkAndDo: function checkAndDo() {
      ...
      $(document).find(".is-hidden.anim-scale").each(function () {
        {
          start_anim($(this));
        }
      });
    ...
    events: function events() {
      ...
      function selects() {
        ...
        $(".filter_select").on("select2:open", function () {
          ...
          $(".select2-results").find(".select2_products-count").remove();
          $(".select2-results").find(".select2_buts").remove();
          var buts = "<div class='select2_buts'><a href='' class='btn btn-gray select2_clear'>Сбросить</a><a href='' class='btn btn-orange select2_set'>Применить</a></div>";
        ...
        $(document).on("click", ".select2_set", function (e) {
          ...
          if ($(".select2-results").find(".select2_products-count").length == 0) {
            $(".select2-results").append('<div class="select2_products-count">Найдено товаров: ' + count + "</div>");
            $(".select2-results").find(".select2_products-count").animate({
              height: "50px"
            }, 300);
          } else {
            $(".select2-results").find(".select2_products-count").text("Найдено товаров: " + count);
          }
        });
      }
      ...
      function sliders() {
        close = function close(item) {
          if ($(".filter_slider_box .filter_slider_base").attr("data-value")) {
            var val = $(".filter_slider_box .filter_slider_base").attr("data-value").split(",");
            $(".filter_slider_box .filter_slider_base").attr("data-slider-value", "[" + val[0] + "," + val[1] + "]");
          }

          $(".filter_slider_box").find(".filter_slider_products-count").remove();
          if ($(".filter_slider_box").length > 0) $(".filter_slider_base").slider("destroy");
          var newHtml = $(".page").find(".filter_slider_box").html();
          item.find(".filter_hide").html(newHtml);
          $(".page").find(".filter_slider_box").remove();
        };

        $(".filter_slider_open").click(function (e) {
          e.preventDefault();
          var html = $(this).find(".filter_hide").html();

          if (!$(this).hasClass("active")) {
            $(".wrapper .page").append('<div class="filter_slider_box">' + html + "</div>");
            $(".page").find(".filter_slider_box").css("left", $(this).offset().left).width($(this).parent().width() - 60);
            $(".page").find(".filter_slider_box").css("top", $(this).offset().top + $(this).height());
            var val = $("input.filter_slider_base").attr("data-slider-value").split(",");
            val[0] = val[0].replace("[", "");
            val[1] = val[1].replace("]", "");
        ...
        $(document).on("click", ".filter_slider_clear", function (e) {
          ...
          var minValue = $(".filter_slider_base").eq(0).attr("data-slider-min");
          var maxValue = $(".filter_slider_base").eq(0).attr("data-slider-max");
          ...
          $(".filter_slider_min span").text(minValue); // $(".filter_slider_base").slider('setValue', defaultValue, true);
        ...
        $(document).on("click", ".filter_slider_set", function (e) {
          ...
          if ($(".filter_slider_box").find(".filter_slider_products-count").length == 0) {
            $(".filter_slider_box").append('<div class="filter_slider_products-count">Найдено товаров: ' + count + "</div>");
            $(".filter_slider_box").find(".filter_slider_products-count").animate({
              height: "50px"
            }, 300);
          } else {
            $(".filter_slider_box").find(".filter_slider_products-count").text("Найдено товаров: " + count);
          }
        });
      }
  ...
  sort: {
    ...
    events: function events() {
      ...
      $(window).on("resize", function () {
        if ($(".catList_typePos").length > 0 && $(".catList_typePos").hasClass("list") && $(window).width() < 768) {
  ...
  mapWhereBuy: {
    ...
    createPlaceMark: function createPlaceMark(coords, src, sity, name, link) {
      ...
      return new ymaps.Placemark(coords, {
        hasBalloon: false // balloonContentHeader:
    ...
    create: function create() {
      ...
      if ($(".map_data .placeMark").length > 0) {
        $(".map_data .placeMark").each(function () {
          clusterer.add(_this.createPlaceMark($(this).attr("data-cords").split(","), $(this).attr("data-img"), $(this).attr("data-sity"), $(this).attr("data-name"), $(this).attr("data-link")));
        });
      }

      myMap.geoObjects.add(clusterer);
      if ($("#map").hasClass("hasBalloon")) myMap.geoObjects.options.set({
        hasBalloon: false
      });

      if ($(window).width() < 768) {// myMap.behaviors.disable("drag");
      }
    ...
    init: function init() {
      ...
      this.center = $(".map_data .placeMark").length > 0 ? $(".map_data .placeMark").eq(0).attr("data-cords").split(",") : [55.751574, 37.573856];
  ...
  rightMenu: {
    ...
    posRightMenu: {
      ...
      setFixed: function setFixed() {
        ...
        var offsetTopFooter = $(".rightMenu-box").offset().top + $(".rightMenu-box").height(); //var offsetTopFooter = $(".footer").offset().top;

        if (offseTop + heightBox >= offsetTopFooter - offsetBottom) {
          var translate = offsetTopFooter - offsetBottom - (offseTop + heightBox);
          $(".rightMenu-container").css("transform", "translateY(" + translate + "px)");
        } else {
          $(".rightMenu-container").css("transform", "translateY(" + 0 + "px)");
        }

        if ($(".rightMenu-container").height() >= $(".rightMenu-box").height() - 120) {
          $(".rightMenu-container").height($(".rightMenu-box").height() - 120);
        } else {
          $(".rightMenu-container").css("height", "");
        }
      },
      clearFixed: function clearFixed() {
        ...
        $(".rightMenu-container").css("width", ""); //$(".rightMenu-container").attr("style", "");
      }
    },
    setFixedInScroll: function setFixedInScroll() {
      var _ = this;
  ...
  cartCalcProd: {
    ...
    refresh: function refresh() {
      ...
      items.each(function () {
        ...
        $(this).find(".summ-value span").text(perfectNum(summ));
        $(this).find(".count-value").text(count);
        allSumm += summ;
      });
  ...
  openOrderRegister: {
    center: null,
    createPlaceMark: function createPlaceMark(coords, id) {
      var width = 400;
      if ($(window).width() <= 600) width = 320;
    ...
    create: function create() {
      ...
      if ($(".map_data .placeMark").length > 0) {
        $(".map_data .placeMark").each(function () {
          var mark = _this.createPlaceMark($(this).attr("data-cords").split(","), $(this).attr("data-id"));
    ...
    events: function events() {
      ...
      $(document).on("click", "#selectAdress", function () {
        var box = $(document).find(".popupOrder-list").find("input:checked").parent();
        ...
        if (val) {
          $(document).find("#setCity label").text(city);
          $(document).find("#setCity input").val(val);
          $(document).find("#indexPosition").val(index);
          $(document).find("#adresPosition").val(adress);
        } else {
          $(document).find("#setCity label").text("Выберите город");
          $(document).find("#setCity input").val("");
          $(document).find("#indexPosition").val("");
          $(document).find("#adresPosition").val("");
        }
    ...
    search: {
      ...
      update: function update() {
        ...
        $(".popupOrder-list").find(".order-check").each(function () {
          var find = false;
  ...
  preloaderInit: function preloaderInit() {
    var imgs = $(".wrapper").find("*:not(script)");
    var items = [];
    imgs.each(function () {
      var url = "";

      if ($(this).css("background-image") != "none" && $(this).css("background-image").split("-")[0] != "linear") {
    ...
    function refreshPreloader() {
      ...
      $({
        numberValue: parseInt($(".preloader_progress .count").text().replace("%", ""))
      }).animate({
        numberValue: percent
      }, {
        duration: 300,
      ...
      if (imgCount == progress) {
        setTimeout(function () {
          document.querySelector(".preloader").className = document.querySelector(".preloader").className + " loaded";
...
var popups = {
  ...
  submits: function submits() {
    $(document).on("submit", "#question form", function (e) {
      ...
      $(document).find('[href="#question_success"]').click();
    });
  },
  init: function init() {
    this.setup();
...
var svgMap = {
  slider: function slider() {
    ...
    var setClasses = function setClasses(slider, id) {
  ...
  setPos: function setPos(target) {
    var imgs = $(".dillerMap_map .box");
    var img = $(".dillerMap_map img");
    var imgOne = $(".dillerMap_map img").eq(0);
    var widthPerfect = imgOne[0].naturalWidth ? imgOne[0].naturalWidth : imgOne.width();
    var heightPerfect = imgOne[0].naturalHeight ? imgOne[0].naturalHeight : imgOne.height();
    ...
    imgs.css("transform", "translate(" + left + "px," + top + "px) scale(" + scale + ")");
  },
  events: function events() {
    var _this = this;
